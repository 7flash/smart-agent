...
export interface AgentConfig {
     ...
     * When exceeded, older messages are summarized to stay within budget.
     * Default: unlimited (no trimming).
     *
     * ```ts
     * maxContextTokens: 100_000  // ~100k tokens
     * ```
     */
    maxContextTokens?: number
    /**
     * Called with periodic progress updates during long-running tool executions.
     * Fires every 5s while a command is still running.
     *
     * ```ts
     * onToolProgress: (tool, elapsed) => console.log(`${tool} still running (${elapsed}s)`)
     * ```
     */
    onToolProgress?: (tool: string, elapsedMs: number) => void
    /** Additional context variables for the exec tool */
...
export type AgentEvent =
    ...
    | { type: "context_trimmed"; iteration: number; originalTokens: number; trimmedTokens: number; removedMessages: number }
    | { type: "tool_progress"; iteration: number; tool: string; elapsedMs: number }
